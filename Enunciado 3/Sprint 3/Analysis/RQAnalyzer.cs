        ...
        public static AnalysisResult Analyze(List<PullRequest> prs, string rqCode)
        {
            ...
            switch (rqCode)
            {
                ...
                case "RQ05":
                    ...
                    result.MedianMerged = StatisticsHelper.Median(prs.Select(p => (double)p.FilesChanged)); // Now holds the independent median
                    result.MedianClosed = StatisticsHelper.Median(allReviewCounts); // Now holds the dependent median
