...
static class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        if (!vaiBuscarLista && !skipFetch && !soAgregar && !soBonus && !soColeta && !File.Exists(reposCsvPath))
        {
            AnsiConsole.MarkupLine("[red]Token GitHub ausente ou lista não existe.[/] Use [cyan].github-token[/], [cyan]GITHUB_TOKEN[/] ou [cyan]--token=...[/]");
            AnsiConsole.MarkupLine("[dim]Coleta CK (sem API): [cyan]--coleta-ck --ck-jar=caminho\\ck.jar[/][/]");
            return;
        }

        if (!skipFetch && !TemToken() && !soColeta)
            AnsiConsole.MarkupLine("[yellow]Sem token:[/] pulando download da lista (use lista já gerada ou force com token).");
        ...
        if (soAgregar || (args.Length == 0))
        {
            if (File.Exists(classCsv) && File.Exists(reposCsvPath))
            {
        ...
        if (!soColeta && vaiBuscarLista)
        {
            ...
            AnsiConsole.MarkupLine("  3. [cyan]dotnet run[/] [dim]— consolidar CSV e gerar relatórios atualizados automaticamente[/]");
